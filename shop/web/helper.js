...
function helper_moment() {
    moment.locale('ru');
    const spns = Array.from(document.querySelectorAll('[data-moment]'));
    for (spn of spns) {
        const tpl = spn.dataset.moment;
        let m = moment(spn.textContent);
        if (!m.isValid()) {
            m = moment.unix(spn.textContent);
        }
        spn.textContent = m.format(tpl);
    }
